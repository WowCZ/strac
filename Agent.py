...
class DialogueAgent(object):
    ...
    def __init__(self, agent_id='Smith', hub_id='dialogueserver', ontology=None, cfg=None, logger=None, SetObj=None):        
        ...
        self.callValidator = CallValidator(cfg, logger)
        ...
        # CONFIGS:
        if cfg.has_option('agent', 'savefrequency'):
            self.SAVE_FREQUENCY = cfg.getint('agent', 'savefrequency')
        if cfg.has_option("agent","systemcanhangup"):
            self.SYSTEM_CAN_HANGUP = cfg.getboolean("agent", "systemcanhangup")
        if cfg.has_option("agent", "maxturns"):
            self.maxTurns_per_domain = cfg.getint("agent", "maxturns")
        if cfg.has_option("GENERAL", "tracedialog"):
            self.traceDialog = cfg.getint("GENERAL", "tracedialog")

        # TOPIC TRACKING:
        #-----------------------------------------
        self.topic_tracker = TopicTracking.TopicTrackingManager(cfg, ontology)
        ...
        # SemO. ****************donnt handle yet
        #-----------------------------------------
        if self.hub_id == 'simulate':      # may or may not have NLG in simulate (default is not to)
            generate_prompts = False
            if cfg.has_option('simulate', 'generateprompts'):
                generate_prompts = cfg.getboolean('simulate', 'generateprompts')
        else:
    ...
    def start_call(self, session_id, domainSimulatedUsers=None, maxNumTurnsScaling=1.0, start_domain=None):
        ...
        self.NUM_DIALOGS += 1
        self.logger.dial(">> NEW DIALOGUE SESSION. Number: "+str(self.NUM_DIALOGS))
        ...
        self.logger.debug('active domain is: '+currentDomain)
        ...
        # print(threading.currentThread().getName() + '  ' + str(id(self.semi_belief_manager)))
        # print(last_sys_act)
        # print(currentDomain)
    ...
    def continue_call(self, asr_info, domainString=None, domainSimulatedUsers=None):
        ...
        self.logger.dial("user input: {}".format([(x.to_string() if isinstance(x,DiaActWithProb) else x[0], round(x.P_Au_O, 3) if isinstance(x,DiaActWithProb) else x[1]) for x in asr_info]))
        ...
        if self.callValidator.check_if_user_bye(asr_info) and not self.callValidator.isValid:
            self.logger.info("User tries to end dialogue before min dialogue length.")
    ...
    def _logical_requirements(self):
        ...
        if self.topic_tracker.USE_SINGLE_DOMAIN:
            if self.policy_manager.domainPolicies[self.topic_tracker.operatingDomain].startwithhello is False:
                self.logger.warning('Enforcing hello() at first turn in singledomain system')
...
#         logger.info('agent: _hand_control')
        ...
        self.ontology.ensure_domain_ontology_loaded(domainString)
        
        if domainString is None:
            self.logger.warning("Topic tracker was unable to resolve domain. Remaining with previous turns operating domain: "\
            ...
            self.logger.info('Launching Dialogue Manager for domain: '+domainString)
            ...
            if domainString not in self.topic_tracker.in_present_dialog:
                ...
                self.logger.info('Handing control from {} to running - {} - dialogue manager'.format(previousDomainString,domainString))                 
                return 
            else:
                # moving back to a domain that occured in an earlier turn of dialog. 
                self.logger.info('Handing control <BACK> to running - '+domainString+' - dialogue manager')
        else:
            self.logger.info('Domain '+domainString+' is both already running and has control') 
        return
    ...
    def _evaluate_agents_turn(self, domainSimulatedUsers=None, sys_act=None, state=None):
        ...
        if self.currentTurn==0:
            self.logger.debug('Note that we are ignoring any evaluation of the systems first action')
            return   
...
#                 self.evaluation_manager.domainEvaluators[operatingDomain].rnn.set_turn_feature(belief,prev_sys_act,turnNo)
        ...
        if self.hub_id=='simulate':
            ...
            if user_model_holder is None:
                self.logger.warning('Simulated user not present for domain %s - passing reward None thru to policy' % operatingDomain)
            else:
    ...
    def _print_turn(self):
        ...
        if self.hub_id=='dialogueserver':
            self.logger.dial('Turn %d' % self.currentTurn)
        else:
    ...
    def _print_sys_act(self, sys_act):
        ...
        '''
        if self.hub_id=='dialogueserver':
            self.logger.dial('Sys > {}'.format(sys_act))
        else:
...
#         assert(not self.ENDING_DIALOG)
        ...
        if self.ENDING_DIALOG:  
            if self.semo_manager is not None:   
                if 'bye' not in self.prompt_str:
                    self.logger.warning('Ignoring system act: %s and saying goodbye as user has said bye' % sys_act)
    ...
    def _agents_semo(self, sys_act):
        ...
        '''
        if self.semo_manager is not None:
            self.logger.dial("Domain with CONTROL: "+self.topic_tracker.operatingDomain)
    ...
    def _save_trainerandreplay(self):
        self.policy_manager.saveTrainerAndReplay()
        return
    ...
    def _check_maxTurns(self):
        ...
        if self.currentTurn > self.maxTurns:
            self.logger.dial("Ending dialog due to MAX TURNS being reached: "+str(self.currentTurn))
    ...
    def _check_agent_not_on_call(self):
        ...
        if self.session_id is not None:
            self.logger.error("Agent is assumed to be only on one call at a time")
    ...
    def _load_manger(self, config, defaultManager):
        ...
        if self.cfg.has_section('agent') and self.cfg.has_option('agent', 'config'):
            manager = self.cfg.has_option('agent', 'config')
        try:
            ...
            print(packageString)
            if packageString == 'policy.PolicyManager':
                return klass(self.ontology, self.cfg, self.logger, self.SetObj)
            else:
                return klass(self.ontology, self.cfg, self.logger)
...
class AgentFactory(object):
    '''
    Based on the config (cfg) - a primary agent (called Smith) is created. 
    ...
    '''
    def __init__(self, hub_id='dialogueserver', ontology=None, cfg=None, logger=None, SetObj=None):
        self.MAX_AGENTS_RUNNING = 2  # always start with 1, but we dont kill agents below this number
        self.init_agents(hub_id, ontology, cfg, logger, SetObj)
        self.session2agent = {}
    ...
    def init_agents(self, hub_id, ontology, cfg, logger, SetObj):
        '''
        ...
        '''
        self.agents = {}
        self.agents['Smith'] = DialogueAgent(agent_id='Smith', hub_id=hub_id, ontology=ontology, cfg=cfg, logger=logger, SetObj=SetObj)  # primary agent, can be copied
    ...
    def start_call(self, session_id, start_domain=None):
        ...
        if session_id in self.session2agent.keys():
            ...
            self.logger.info('Attempted to start a call with a session_id %s already in use by agent_id %s .' % (session_id, agent_id))
        ...
        if agent_id is None:
            ...
            self.logger.info('Agent {} has been reactivated.'.format(agent_id))
    ...
    def end_call(self, agent_id=None, session_id=None, noTraining=False):
        ...
        self.logger.info('Ending agents %s call' % agent_id)
    ...
    def retrieve_agent(self, session_id):
        ...
        if session_id not in self.session2agent.keys():
            self.logger.error('Attempted to get an agent for unknown session %s' % session_id)
    ...
    def query_ENDING_DIALOG(self, agent_id):
        ...
        if agent_id not in self.agents.keys():
            self.logger.error('Not an existing agent: '+str(agent_id))
...
#         self.agents[agent_id] = copy.deepcopy(self.agents['Smith']) # alternative to copying is a new DialogueAgent() object  
        self.agents[agent_id] = DialogueAgent(agent_id, ontology=self.ontology, cfg=self.cfg, logger=self.logger)
        self.agents[agent_id].restart_agent(session_id=None) # VERY IMPORTANT AFTER copying!!
        self.logger.info('Agent {} has been created.'.format(agent_id))
        return agent_id
    ...
    def power_down_factory(self, train):
        '''
        ...
        for agent_id in self.agents.keys():
            self.logger.info('Summary of agent: %s' % agent_id)
            ...
            if train:
                ...
                self.agents[agent_id]._save_trainerandreplay()
        self.logger.info('Factory handled these sessions: %s' % self.historical_sessions)
...
class CallValidator(object):
    ...
    def __init__(self, cfg, logger):
        ...
        if cfg.has_option('agent', 'mindialoguedurationprompt'):
            self.mindialoguedurationprompt = cfg.get('agent', 'mindialoguedurationprompt')
        if cfg.has_option('agent', 'mindialogueturns'):
            self.mindialogueturns = cfg.getint('agent', 'mindialogueturns')
        if cfg.has_option('agent', 'mindialogueduration'):
            self.mindialogueduration = cfg.getint('agent', 'mindialogueduration')
    ...
    def validate(self, sys_act = None):
        ...
        if timeStatus:
            if self.doValidityCheck:
                self.logger.info("Call is valid due to time ({} sec passed).".format(timePassed))
            self.isValid = True
            self.isTrainable = True
        
        if sysActStatus:
            if self.doValidityCheck:
                self.logger.info("Call is valid due to system act.")
            self.isValid = True
            self.isTrainable = True
            
        if turnStatus:
            if self.doValidityCheck:
                self.logger.info("Call is valid due to number of turns ({} turns).".format(self.turns))
            self.isValid = True
            self.isTrainable = True
