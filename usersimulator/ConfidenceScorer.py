...
class DSTC2ConfidenceScorer(object):
    ...
    def __init__(self, logger, SetObj, paramset=None):
    ...
    def assignConfScores(self, dapl):
        ...
        for i in range(len(dapl)):
            sample = self.SetObj.random.normal(u[i], max(0.0001,std[i]))
        ...
        for i in range(len(dapl), len(u)):
            sample = self.SetObj.random.normal(u[i], max(0.00001, std[i]))
            sample = round(max(min(0.99, sample), 0.01), 4)
            rest_cs += sample
