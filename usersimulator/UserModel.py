...
class UMAgenda(object):
    ...
    :type domain: str
    '''
    def __init__(self, dstring, ontology, logger, SetObj):
    ...
    def init(self, goal):
        ...
        for const in goal.constraints:
            ...
            slots = self.ontology.getSlotsToExpress(self.dstring, slot, value)
    ...
    def _conditionally_init_agenda(self, goal):
        ...
        for dact in goal.copied_constraints:
            ...
            uniform_sample = self.SetObj.random.uniform()
            ...
            if uniform_sample < self.CONFIRM:
                ...
                self.logger.info("changing inform() act to a confirm() act")
                self.replace_acts_slot(dact.slot, replaceact="confirm") 
            elif uniform_sample < self.LEAVE_INFORM:
                pass
            else:
                self.logger.info("removing the inform() act and not replacing with anything.") 
...
class UMGoal(object):
    ...
    :type patience: int
    '''
    def __init__(self, patience, domainString, request_type):
        # self.ontology = ontology
        self.constraints = []
        ...
        # self.request_type = self.ontology.get_type(domainString)  
        self.request_type = request_type

        self.system_has_informed_name_none = False
        self.no_relaxed_constraints_after_name_none = False

    def clear(self, patience, domainString, request_type):
        self.constraints = []
        self.requests = {}
        self.prev_slot_values = {}
        self.patience = patience
...
class GoalGenerator(object):
    ...
    '''
    def __init__(self, dstring, ontology, cfg, SetObj):
        '''
        ...
        if cfg.has_option('goalgenerator','maxvenuespergoal'):
            configlist.append('maxvenuespergoal')
            self.MAX_VENUES_PER_GOAL = cfg.getint('goalgenerator','maxvenuespergoal')
        self.MIN_VENUES_PER_GOAL = 1

        self.MAX_CONSTRAINTS = 3
        if cfg.has_option('goalgenerator','maxconstraints'):
            configlist.append('maxconstraints')
            self.MAX_CONSTRAINTS = cfg.getint('goalgenerator','maxconstraints')

        self.MAX_REQUESTS = 3
        if cfg.has_option('goalgenerator','maxrequests'):
            configlist.append('maxrequests')
            self.MAX_REQUESTS = cfg.getint('goalgenerator','maxrequests')
        self.MIN_REQUESTS = 1
        if cfg.has_option('goalgenerator','minrequests'):
            configlist.append('minrequests')
            self.MIN_REQUESTS = cfg.getint('goalgenerator','minrequests')
        assert(self.MIN_REQUESTS > 0)
        if cfg.has_option('goalgenerator','minvenuespergoal'):
            self.MIN_VENUES_PER_GOAL = int(cfg.get('goalgenerator','minvenuespergoal'))
#        self.PERCENTAGE_OF_ZERO_SOLUTION_TASKS = 50
#        if self.cfg.has_option('GOALGENERATOR','PERCZEROSOLUTION'):
#            self.PERCENTAGE_OF_ZERO_SOLUTION_TASKS = int(self.cfg.get('GOALGENERATOR','PERCZEROSOLUTION'))
#        self.NO_REQUESTS_WITH_VALUE_NONE = False
#        if self.cfg.has_option('GOALGENERATOR','NOREQUESTSWITHVALUENONE'):
#            self.NO_REQUESTS_WITH_VALUE_NONE
    ...
    def init_goal(self, otherDomainsConstraints, um_patience):
        ...
        request_type = self.ontology.get_type(self.dstring)
        goal = UMGoal(um_patience, domainString=self.dstring, request_type=request_type)
        ...
        while True:
            ...
            venues = self.ontology.entity_by_features(self.dstring, constraints=goal.constraints)
            ...
            if num_attempts_to_resample == 0:
                self.logger.warning('Maximum number of goal resampling attempts reached.')
    ...
    def _set_other_domains_constraints(self, otherDomainsConstraints):
        ...
        valid_const_slots = self.ontology.getValidSlotsForTask(self.dstring) 
        ...
        for const in otherDomainsConstraints:
            if const.slot in valid_const_slots and const.val != "dontcare": #TODO think dontcare should be dealt with diff 
                ...
                #TODO delete: if const.val in self.ontology.ontology['informable'][const.slot]: #make sure value is valid for slot
                if self.ontology.is_value_in_slot(self.dstring, value=const.val, slot=const.slot):
    ...
    def _init_consts_requests(self, goal, um_patience):
        ...
        '''
        request_type = self.ontology.get_type(self.dstring)
        goal.clear(um_patience, domainString=self.dstring, request_type=request_type)

        # Randomly pick a task: bar, hotel, or restaurant (in case of TownInfo)
        goal.request_type = self.ontology.getRandomValueForSlot(self.dstring, slot='type')

        # Get a list of slots that are valid for this task.
        valid_const_slots = self.ontology.getValidSlotsForTask(self.dstring)
        
        # First randomly sample some slots from those that are valid: 
        sampling_probs = self.ontology.get_sample_prob(self.dstring, 
                                                                  candidate=valid_const_slots,
                                                                  conditional_values=self.conditional_constraint_slots)
        random_slots = list(self.SetObj.random.choice(valid_const_slots,
        ...
        for slot in random_slots:
            goal.add_const(slot, self.ontology.getRandomValueForSlot(self.dstring, slot=slot, 
                                                            nodontcare=False,
        ...
        if self.MIN_REQUESTS == self.MAX_REQUESTS:
            ...
            n = self.SetObj.random.randint(low=self.MIN_REQUESTS-1,high=self.MAX_REQUESTS)
        valid_req_slots = self.ontology.getValidRequestSlotsForTask(self.dstring)
        if n > 0 and len(valid_req_slots) >= n:   # ie more requests than just 'name'
            choosen = self.SetObj.random.choice(valid_req_slots, n,replace=False)
...
class UM(object):
    ...
    :param None:
    '''
    def __init__(self, domainString, ontology, cfg, logger, SetObj):
        self.ontology = ontology
        self.cfg = cfg
        self.logger = logger
        self.SetObj = SetObj
        ...
        if self.cfg.has_option('goalgenerator', 'patience'): # only here for backwards compatibility; should actually be in um
            self.max_patience = self.cfg.get('goalgenerator', 'patience')
        if self.cfg.has_option('usermodel', 'patience'):
            self.max_patience = self.cfg.get('usermodel', 'patience')
        if isinstance(self.max_patience,str) and len(self.max_patience.split(',')) > 1:
            ...
            if len(self.sample_patience) != 2 or self.sample_patience[0] > self.sample_patience[1]:
                self.logger.error('Patience should be either a single int or a range between 2 ints.')
        else:
            self.max_patience = int(self.max_patience)
        
        self.patience_old_style = False
        if self.cfg.has_option('usermodel', 'oldstylepatience'):
            self.patience_old_style = self.cfg.getboolean('usermodel', 'oldstylepatience')
        self.old_style_parameter_sampling = True
        if self.cfg.has_option('usermodel', 'oldstylesampling'):
            self.old_style_parameter_sampling = self.cfg.getboolean('usermodel', 'oldstylesampling')
            
        self.sampleParameters = False
        if self.cfg.has_option('usermodel', 'sampledialogueprobs'):
            self.sampleParameters = self.cfg.getboolean('usermodel', 'sampledialogueprobs')
    ...
    def init(self, otherDomainsConstraints):
        ...
        if self.sample_patience:
            self.max_patience = self.SetObj.random.randint(self.sample_patience[0], self.sample_patience[1])
        
        self.goal = self.generator.init_goal(otherDomainsConstraints, self.max_patience)
        self.logger.debug(str(self.goal))
    ...
    def receive(self, sys_act):
        ...
        if self.lastUserAct is not None and self.lastUserAct.act == 'repeat' and\
            ...
            self.logger.info("Endless cycle of repeating repeats. Setting patience to zero.")
        ...
        if self.goal.patience < 1:
            self.logger.debug(str(self.goal))
            self.logger.debug('All patience gone. Clearing agenda.')
    ...
    def _load_domain_goal_generator(self, domainString):
        ...
        if self.cfg.has_option('usermodel_' + domainString, 'goalgenerator'):
            generatorClass = self.cfg.get('usermodel_' + domainString, 'goalgenerator')
        
        if generatorClass is None:
            return GoalGenerator(domainString, self.ontology, self.cfg, self.SetObj)
        else:
            try:
                ...
                return klass(domainString, self.ontology)
            except ImportError:
                self.logger.error('Unknown domain ontology class "{}" for domain "{}"'.format(generatorClass, domainString))
    ...
    def _load_domain_simulator(self, domainString):
        ...
        if self.cfg.has_option('usermodel_' + domainString, 'usersimulator'):
            simulatorClass = self.cfg.get('usermodel_' + domainString, 'usersimulator')
        
        if simulatorClass is None:
            return UMHdcSim.UMHdcSim(domainString, self.ontology, self.cfg, self.logger, self.SetObj)
        else:
            try:
                # try to view the config string as a complete module path to the class to be instantiated
                ...
                self.logger.error('Unknown domain ontology class "{}" for domain "{}"'.format(simulatorClass, domainString))
