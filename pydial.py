...
def getOptionalConfigVar(configvarname, cfg, default='', section='exec_config'):
    value = default
    if cfg.has_option(section, configvarname):
        value = cfg.get(section, configvarname)
    return value

def getRequiredDirectory(directoryname, cfg, section='exec_config'):
    assert cfg.has_option(section, directoryname),\
        "Value {} in section {} is missing.".format(directoryname, section)
    dir = cfg.get(section, directoryname)
    if dir[-1] != '/': dir = dir+'/'
    return dir

def getOptionalConfigInt(configvarname, cfg, default='0',section='exec_config'):
    value = default
    if cfg.has_option(section, configvarname):
        value = cfg.getint(section, configvarname)
    return value

def getOptionalConfigBool(configvarname, cfg, default='False', section='exec_config'):
    value = default
    if cfg.has_option(section, configvarname):
        value = cfg.getboolean(section, configvarname)
    return value


def setupPolicy(domain, configId, trainerr, source_iteration, target_iteration, cfg, policy_dir, seed=None):
    curThreadName = threading.currentThread().getName()
    universal_configId = configId.split('-')[:-1]
    universal_configId[0] = ''.join(curThreadName.split('-'))
    universal_configId.append('UNIVERSAL')
    universal_configId = '-'.join(universal_configId)
    # print('#############################')
    # print(universal_configId)
    # exit(0)

    if cfg.has_section("policy_" + domain):
    ...
    if not str(source_iteration).isdigit():
        ...
        inpolicyfile = "%s-seed%s-%02d.%d" % (universal_configId, seed, trainerr, source_iteration)
        outpolicyfile = "%s-seed%s-%02d.%d" % (universal_configId, seed, trainerr, target_iteration)
    else:
        inpolicyfile = "%s-%02d.%d" % (universal_configId, trainerr, source_iteration)
        outpolicyfile = "%s-%02d.%d" % (universal_configId, trainerr, target_iteration)

    cfg.set(policy_section, "inpolicyfile", policy_dir + inpolicyfile)
    cfg.set(policy_section, "outpolicyfile", policy_dir + outpolicyfile)
    
    return (inpolicyfile,outpolicyfile)



def setEvalConfig(domain, configId, evalerr, ndialogs, iteration, cfg, policy_dir, traceDialog, conf_dir, policytype, logger, seed=None):
    (_, policy) = setupPolicy(domain, configId, evalerr, iteration, iteration, cfg, policy_dir,seed=seed)
    ...
    if cfg.has_section("policy_" + domain):
        cfg.set("policy_" + domain, "learning", 'False')
    else:
        cfg.set("policy", "learning", 'False')
    # if gp, make sure to reset scale to 1 for evaluation
    if policytype == "gp":
        if cfg.has_section("gpsarsa_" + domain):
            cfg.set("gpsarsa_" + domain, "scale", "1")
        else:
            cfg.set("gpsarsa", "scale", "1")
    # Save a copy of config file
    # confsavefile = conf_dir + "%s.eval.%02d.cfg" % (policy, evalerr)
    # cf = open(confsavefile, 'w')
    # cfg.write(cf)

def evalPolicy(domain, configId, evalerr, ndialogs, iteration, ontology, cfg, policy_dir, traceDialog, conf_dir, policytype, logger, SetObj, seed=None):
    setEvalConfig(domain, configId, evalerr, ndialogs, iteration, cfg, policy_dir, traceDialog, conf_dir, policytype, logger, seed=seed)

    error = float(evalerr) / 100.0
    # finally run the system
    # ndialogs = 100
    simulator = Simulate.SimulationSystem(error_rate=error, ontology=ontology, cfg=cfg, logger=logger, SetObj=SetObj)
    simulator.run_dialogs(ndialogs, False)
...
def extractEvalData(lines):
    ...
    if lines[0].find('Evaluating') >= 0:
        ...
        for domain in domain_list:
            iteration = 0
            evalData[domain][0] = {}
            evalData[domain][0]['erate'] = getIntParam(lines[0], 'error-rate')
            evalData[domain][0]['ndialogs'] = 0
            training = True
...
def plotTrain(dname,rtab,stab,block=True,saveplot=False):
    ...
    for policy in policylist:
        ...
        # domainEnv = policyList[-2]+'-'+policyList[0]
        domainEnv = policyList[0]
        etype = policyList[1]+'-'+policyList[2]
        for i in range(len(tab['x'])):
            for j in range(len(tab['x'][i])):
                df_dict['Num_Dialogs'].append(tab['x'][i][j])
                df_dict['Reward'].append(tab['y'][i][j])
                df_dict['var'].append(tab['var'][i][j])
                df_dict['domainEnv'].append(domainEnv)
                df_dict['etype'].append(etype)

    df = pd.DataFrame.from_dict(df_dict)

    # kws = dict(elinewidth=2, capsize=2)
    g = sns.FacetGrid(df, col="domainEnv", hue="etype", col_wrap=3)
    g = (g.map(sns.lineplot, "Num_Dialogs", "Reward").set(ylim=(-5, 15)).add_legend())
    plt.savefig('czresult_plot/' + dname + '.png', bbox_inches='tight')
    # plt.show()
        ...
        for policy_key in result:
            ...
            for key in result[policy_key]:
                if not key in averaged_result[general_policy_key]:
                    averaged_result[general_policy_key][key] = [result[policy_key][key]]
                else:
                    averaged_result[general_policy_key][key].append(result[policy_key][key])
...
def init(configId, config_file, seed, mode, trainerrorrate=None, trainsourceiteration=None,
               ...
               dbprefix=None, main_threadID=None):

    if seed is not None:
        seed = int(seed)
    SetObj = Settings.Settings(config_file, seed)
    seed = SetObj.seed
    taskID='ID'
    cfg = SetObj.config
    ...
    isSingleDomain = getOptionalConfigBool("singledomain", cfg, isSingleDomain, "GENERAL")
    isSingleModel = getOptionalConfigBool("singlemodel", cfg, False, "policycommittee")
    traceDialog    = getOptionalConfigInt("tracedialog", cfg, tracedialog, "GENERAL")
    domain         = getOptionalConfigVar("domains", cfg, '', "GENERAL")
    # print('$$$$$$' + str(domain))
    curThreadName = threading.currentThread().getName()
    print(curThreadName + ' ' + str(domain))
    Settings.global_traincontroller[curThreadName] = False
    # print(isSingleModel)
    # exit(0)
    ...
    #     print('Parallel Training Starts!')
    ...
    if isSingleDomain:
        if cfg.has_section('policy_' + domain):
            policytype = getOptionalConfigVar('policytype', cfg, policytype, 'policy_' + domain)
        else:
            policytype = getOptionalConfigVar('policytype', cfg, policytype, 'policy')
        conventionCheck(configId)
    else:
        domains = getOptionalConfigVar("domains", cfg, "", "GENERAL").split(',')
        policytypes = {}
        for domain in domains:
            if cfg.has_section('policy_' + domain):
                policytypes[domain] = getOptionalConfigVar('policytype', cfg, policytype, 'policy_' + domain)
            else:
                policytypes[domain] = getOptionalConfigVar('policytype', cfg, policytype, 'policy')

    # if gp, make sure to save required scale before potentially overriding
    if isSingleDomain:
        if policytype == "gp":
            if cfg.has_section("gpsarsa_" + domain):
                gpscale = cfg.getint("gpsarsa_" + domain, "scale")
            else:
                gpscale = cfg.getint("gpsarsa", "scale")
    else:
        gpscales = {}
        for domain in domains:
            if policytypes[domain] == "gp":
                if cfg.has_section("gpsarsa_" + domain):
                    gpscales[domain] = cfg.getint("gpsarsa_"+ domain, "scale")
                else:
                    gpscales[domain] = cfg.getint("gpsarsa", "scale")
    # if deep-rl model, make sure to set the correct n_in
    if isSingleDomain:
        if cfg.has_section("dqnpolicy"):
            if domain == 'CamRestaurants':
                cfg.set("dqnpolicy", 'n_in', 268)
            elif domain == 'SFRestaurants':
                cfg.set("dqnpolicy", 'n_in', 636)
            elif domain == 'Laptops11':
                cfg.set("dqnpolicy", 'n_in', 257)
                # TODO: set rest of environments and multidomain

    # Get required folders and create if necessary
    log_dir    = getRequiredDirectory("logfiledir", cfg)
    conf_dir   = getRequiredDirectory("configdir", cfg)
    if isSingleDomain:
        if policytype != 'hdc':
            if cfg.has_section("policy_"+domain):
                policy_dir = getRequiredDirectory("policydir", cfg,"policy_"+domain)
            else:
                policy_dir = getRequiredDirectory("policydir", cfg, "policy")
            pd = path(policy_dir)
            if not pd.isdir():
    ...
    if numtrainbatches:
        ...
        gnumtrainbatches = getOptionalConfigInt("numtrainbatches", cfg, 1)
    if traindialogsperbatch:
        gtraindialogsperbatch = int(traindialogsperbatch)
    else:
        gtraindialogsperbatch = getOptionalConfigInt("traindialogsperbatch", cfg, 100)
	#gtraindialogsperbatch = 50
    if trainerrorrate:
        gtrainerrorrate = int(trainerrorrate)
    else:
        gtrainerrorrate = getOptionalConfigInt("trainerrorrate", cfg, 0)
    if testerrorrate:
        gtesterrorrate = int(testerrorrate)
    else:
        gtesterrorrate = getOptionalConfigInt("testerrorrate", cfg, 0)
    if trainsourceiteration:
        gtrainsourceiteration = int(trainsourceiteration)
    else:
        gtrainsourceiteration = getOptionalConfigInt("trainsourceiteration", cfg, 0)
    if numtestdialogs:
        gnumtestdialogs = int(numtestdialogs)
    else:
        gnumtestdialogs = getOptionalConfigInt("numtestdialogs", cfg, 50)
	#gnumtestdialogs = 100

    gnumbatchtestdialogs = getOptionalConfigInt("numbatchtestdialogs", cfg, 20)
    #gnumbatchtestdialogs = 100
    gtesteverybatch = getOptionalConfigBool("testeverybatch", cfg, True)
    gdeleteprevpolicy = getOptionalConfigBool("deleteprevpolicy", cfg, False)
    ...
    cfg.set("logging", "file", log_dir + logfile)
    if traindomains:
        cfg.set("GENERAL", "traindomains", traindomains)
    if testdomains:
        cfg.set("GENERAL", "testdomains", testdomains)
    if dbprefix:
        cfg.set("exec_config", "dbprefix", dbprefix)

    current_threadID = str(threading.currentThread().getName())
    # if not Ontology.global_ontology:
    logger = ContextLogger.getLoggerThread(main_threadID+'.'+current_threadID, config=cfg)
    # print(threading.currentThread().getName() + ': ' +str(id(logger)))
    # exit(0)

    ontology = Ontology.init_ontology_utils(cfg, SetObj)
    # else:
    #     ContextLogger.resetLoggingHandlers()
    #     ContextLogger.createLoggingHandlers(config=Settings.config)
    #     logger = ContextLogger.getLogger('')

    SetObj.random.seed(int(seed))
    if SetObj.root=='':
        SetObj.root = os.getcwd()


    logger.info("Seed = %d", seed)
    logger.info("Root = %s", SetObj.root)


    return (logger, logfile, traceDialog, isSingleDomain, policy_dir, conf_dir, log_dir, \
           gnumtrainbatches, gtraindialogsperbatch, gnumbatchtestdialogs, gnumtestdialogs, \
           gtrainerrorrate, gtesterrorrate, gtrainsourceiteration, \
           taskID, domain, domains, policytype, gtesteverybatch, gpscale, \
           gdeleteprevpolicy, isSingleModel, policytypes, ontology, cfg, SetObj)

def training(isSingleDomain, logger, domain, configId, trainerr, ndialogs, source_iteration, ontology,
             gdeleteprevpolicy, cfg, policy_dir, traceDialog, conf_dir, policytype, SetObj, seed=None):
    (inpolicy, outpolicy) = setupPolicy(domain, configId, trainerr, source_iteration, source_iteration + 1, 
                            cfg, policy_dir, seed=seed)
    ...
    if cfg.has_section("policy_" + domain):
        cfg.set("policy_" + domain, "learning", 'True')
    else:
        cfg.set("policy", "learning", 'True')
    # if gp, make sure to reset scale to config setting
    if policytype == "gp":
        if cfg.has_section("gpsarsa_" + domain):
            cfg.set("gpsarsa_" + domain, "scale", str(gpscale))
        else:
            cfg.set("gpsarsa", "scale", str(gpscale))
    ...
    # ndialogs = 20
    simulator = Simulate.SimulationSystem(error_rate=error, ontology=ontology, cfg=cfg, logger=logger, SetObj=SetObj)
    simulator.run_dialogs(ndialogs, True)
    if gdeleteprevpolicy:
        if inpolicy[-1] != '0':
            if cfg.has_section("policy_" + domain):
                for f in os.listdir(cfg.get('policy_{}'.format(domain),'policydir')):
                    if re.search(inpolicy, f):
                        os.remove(os.path.join(cfg.get('policy_{}'.format(domain),'policydir'), f))
            else:
                for f in os.listdir(cfg.get('policy','policydir')):
                    if re.search(inpolicy, f):
                        os.remove(os.path.join(cfg.get('policy','policydir'), f))


def TrainThread(configfile, seed, trainerrorrate, trainsourceiteration,
                  numtrainbatches,traindialogsperbatch,traindomains,dbprefix, main_threadID):
    # print('************ '+ threading.currentThread().getName() + '\n')

    configId = getConfigId(configfile)
    logger, logfile, traceDialog, isSingleDomain, policy_dir, conf_dir, log_dir, \
    gnumtrainbatches, gtraindialogsperbatch, gnumbatchtestdialogs, gnumtestdialogs, \
    gtrainerrorrate, gtesterrorrate, gtrainsourceiteration, \
    taskID, domain, domains, policytype, gtesteverybatch, gpscale, \
    gdeleteprevpolicy, isSingleModel, policytypes, ontology, cfg, SetObj = init(configId,configfile,seed,"train",trainerrorrate=trainerrorrate,
               trainsourceiteration=trainsourceiteration,numtrainbatches=numtrainbatches,
               traindialogsperbatch=traindialogsperbatch,traindomains=traindomains,dbprefix=dbprefix, main_threadID=main_threadID)

    evalPolicy(domain, configId, gtrainerrorrate, gnumbatchtestdialogs, 0, ontology, cfg,
               policy_dir, traceDialog, conf_dir, policytype, logger, SetObj, seed=seed)
    for i in range(gtrainsourceiteration,gtrainsourceiteration+gnumtrainbatches):
        logger.results('List of domains: {}'.format(domain))
        training(isSingleDomain, logger, domain, configId, gtrainerrorrate, gtraindialogsperbatch, i, \
                 ontology, gdeleteprevpolicy, cfg, policy_dir, traceDialog, conf_dir, policytype, SetObj, seed=seed)
        if gtesteverybatch and gnumbatchtestdialogs>0 and i+1 < gtrainsourceiteration+gnumtrainbatches:
            evalPolicy(domain, configId, gtrainerrorrate, gnumbatchtestdialogs, i + 1, ontology, cfg, 
                       policy_dir, traceDialog, conf_dir, policytype, logger, SetObj, seed=seed)
...
def parallelTrain_command(configfile1, configfile2, configfile3, seed=None, trainerrorrate=None,trainsourceiteration=None,
                  numtrainbatches=None,traindialogsperbatch=None,traindomains=None,dbprefix=None):
    """ Train a policy parallelly according to the supplied configfiles.
        Results are stored in the directories specified in the [exec_config] section of the config files.
        Optional parameters over-ride the corresponding config parameters of the same name.
        The default is the seeting that the domain is single here and the seed is an integer.
    """

    import tube
    tube.seed = seed

    main_threadID = str(threading.currentThread().getName())
    # main_logger = ContextLogger.getLoggerThread(name=main_threadID,log_file='main.log')
    ...
    try:
        thread = threading.Thread(target=TrainThread, args=(configfile1,seed, \
                trainerrorrate,trainsourceiteration,numtrainbatches,traindialogsperbatch,traindomains,dbprefix, main_threadID))
        thread.start()
        # thread.join(5)
        time.sleep(5)

        thread = threading.Thread(target=TrainThread, args=(configfile2,seed, \
                trainerrorrate,trainsourceiteration,numtrainbatches,traindialogsperbatch,traindomains,dbprefix, main_threadID))
        thread.start()
        # thread.join(5)
        time.sleep(5)

        thread = threading.Thread(target=TrainThread, args=(configfile3,seed, \
                trainerrorrate,trainsourceiteration,numtrainbatches,traindialogsperbatch,traindomains,dbprefix, main_threadID))
        thread.start()
        # thread.join(5)
        time.sleep(5)
...
def train_command(configfile, seed=None, trainerrorrate=None,trainsourceiteration=None,
    ...
    try:
        if seed and seed.startswith('('):
            ...
            evalPolicy(domain, configId, gtrainerrorrate, gnumbatchtestdialogs, 0, seed=seed)
            for i in range(gtrainsourceiteration,gtrainsourceiteration+gnumtrainbatches):
                if isSingleDomain:
                    logger.results('List of domains: {}'.format(domain))
