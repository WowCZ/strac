...
if __name__ == '__main__':
    def worker(message, filename):
        GeneralLogger().set_log_path(filename)
    ...
    GeneralLogger().set_log_path('test.txt')
    GeneralLogger().set_log_by_thread_log(True)
    GeneralLogger().set_log_level(LOG_LEVEL_DEBUG)
    main_logger = GeneralLogger().get_logger()
    main_logger.debug('debug')
    ...
    main_logger.info('info')
    main_logger.error('error')
    t1 = threading.Thread(target=worker, args=('worker 1','/home/slhome/zc825/wowcz_github/Pydial/utils/test1.txt'))
    t2 = threading.Thread(target=worker, args=('worker 2','/home/slhome/zc825/wowcz_github/Pydial/utils/test2.txt'))
    t1.start()
    t2.start()