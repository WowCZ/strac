...
def getLoggerThread(name, config=None, screen_level = "RESULTS", \
                          log_file = None, file_level = "RESULTS", use_color = True):
    """
    Retrieve or if necessary create a context logger with specified name.
    ...
    .. note::
        Use **only** this function to create instances of the ContextLogger class
    """
    cl_thread = {}             # current set of context loggers indexed by module name
    module_level_thread = {}   # logging level for each logger in cl
    ...
    if config:
        ...
        for option in config.options('logging'):
            if option not in ['usecolor','file', 'file_level', 'screen_level'] and option not in config.defaults():
                logger_name = option.lower()
                module_level_thread[logger_name] = config.get('logging', option)
                if logger_name in cl_thread:
                    cl_thread[logger_name].setLevel(module_level_thread[logger_name])
    ...
    # name = name.lower()
    if name not in cl_thread:
        cl_thread[name] = ContextLogger(name)
        if name in module_level_thread:
            cl_thread[name].setLevel(module_level_thread[name])

    return cl_thread[name]
