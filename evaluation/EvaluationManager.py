...
class EvaluationManager(object):
    ...
    def __init__(self, ontology, cfg, logger):
        ...
        self.logger = logger
        self.domainEvaluators = dict.fromkeys(ontology.OntologyUtils.available_domains, None)
        self.final_reward = dict.fromkeys(ontology.OntologyUtils.available_domains, None)
    ...
    def _load_domains_evaluator(self, domainString=None):
        ...
        evaluator = 'objective'
        if self.cfg.has_option('eval', 'successmeasure'):
            evaluator = self.cfg.get('eval', 'successmeasure')
        if self.cfg.has_option('eval_' + domainString, 'successmeasure'):
            evaluator = self.cfg.get('eval_' + domainString, 'successmeasure')
...
#         elif
        if evaluator == "objective": 
            ...
            self.domainEvaluators[domainString] = SuccessEvaluator.ObjectiveSuccessEvaluator(domainString, self.ontology, self.cfg, self.logger)
        elif evaluator == "subjective": 
            import SuccessEvaluator
            self.domainEvaluators[domainString] = SuccessEvaluator.SubjectiveSuccessEvaluator(domainString, self.ontology, self.cfg, self.logger)
        else:
            try:
                ...
                self.logger.error('Unknown domain evaluator "{}" for domain "{}"'.format(evaluator, domainString))
    ...
    def _bootup_domain(self, dstring):
        ...
        if dstring in self.SPECIAL_DOMAINS:
            self.logger.warning("No eval manager a present for {}".format(dstring))
            return
