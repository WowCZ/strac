...
class MultiEvaluator(Evaluator):
    ...
    def __init__(self, domainString, cfg):
        super(MultiEvaluator, self).__init__(domainString, cfg)
        
        self.evaluators = [ObjectiveSuccessEvaluator(domainString), SubjectiveSuccessEvaluator(domainString)]
        
        self.evaluatorList = None
        if cfg.has_option('eval', 'multievaluatorlist'):
            self.evaluatorList = cfg.get('eval', 'multievaluatorlist').split(',')
        if cfg.has_option('eval_'+domainString, 'multievaluatorlist'):
            self.evaluatorList = cfg.get('eval_'+domainString, 'multievaluatorlist').split(',')
...
#             self.evaluator_label += e.evaluator_label + " "
        ...
        if cfg.has_option('eval', 'mainevaluator'):
            self.mainEvaluator = cfg.getint('eval', 'mainevaluator')
        if cfg.has_option('eval_'+domainString, 'mainevaluator'):
            self.mainEvaluator = cfg.getint('eval_'+domainString, 'mainevaluator')
            
        self.compareEvaluator = None
        if cfg.has_option('eval', 'compareevaluator'):
            self.compareEvaluator = cfg.getint('eval', 'compareevaluator')
        if cfg.has_option('eval_'+domainString, 'compareevaluator'):
            self.compareEvaluator = cfg.getint('eval_'+domainString, 'compareevaluator')
