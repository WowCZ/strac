...
class FlatDomainOntology(object):
    ...
    '''
    def __init__(self, domainString, cfg, OntologyUtils, SetObj, rootIn=None):
        ...
        if rootIn is not None:
            self.SetObj.load_root(rootIn)  # for use with semi file parser - where no config is given to set repository root by

        self.cfg = cfg
        self.domainString = domainString
        self.OntologyUtils = OntologyUtils
    ...
    def _set_ontology(self):
        ...
        """
        # print('@@@@@@@@@@@@'+self.domainString)
        # exit(0)
        ontology_fname = self.OntologyUtils.get_ontology_path(self.domainString)
    ...
    def _set_db(self):
        ...
        """
        db_fname = self.OntologyUtils.get_database_path(self.domainString)
        logger.info('Loading database: '+db_fname+'db')
        try:
            #self.db = DataBase.DataBase(db_fname+'txt')
            dbprefix = None
            if self.cfg.has_option("exec_config", "dbprefix"):
                dbprefix = self.cfg.get("exec_config", "dbprefix")
                if dbprefix.lower() == 'none':
                    dbprefix = None
            if dbprefix:
                db_fname = os.path.join(dbprefix, db_fname.split('/')[-1])
            self.db = DataBaseSQLite.DataBase_SQLite(dbfile=db_fname+'db', dstring=self.domainString, SetObj=self.SetObj)
        except IOError:
            print IOError
    ...
    def getRandomValueForSlot(self, slot, nodontcare=False, notthese=[], conditional_values=[]):
        ...
        return self.SetObj.random.choice(candidate, p=conditional_sample_prob)
...
class FlatOntologyManager(object):
    ...
    def __init__(self, cfg, OntologyUtils, SetObj):
        self.cfg = cfg
        self.OntologyUtils = OntologyUtils
        self.SetObj = SetObj
    ...
    def _ensure_singleton(self):
        ...
        #     logger.error(msg)
        print(FlatOntologyManager.instances)
        return
    ...
    def _config_bootup(self):
        ...
        '''
        if not self.cfg.has_option("GENERAL","domains"):
            logger.error("You must specify the domains (a domain) under the GENERAL section of the config")
        domains = self.cfg.get("GENERAL",'domains')
        if domains in ['camtourist','sftourist','electronics','all']:
            self.possible_domains = self.OntologyUtils.get_domains_group(domains)
        else:
    ...
    def updateBinarySlots(self, dstring):
        if 'binary' in self.ontologyManagers[dstring].ontology:
            self.OntologyUtils.BINARY_SLOTS[dstring] = self.ontologyManagers[dstring].ontology['binary']
        else:
            self.OntologyUtils.BINARY_SLOTS[dstring] = []
    ...
    def entity_by_features(self, dstring, constraints): 
        if self.ontologyManagers[dstring] is not None:
            lock.acquire()
            entities = self.ontologyManagers[dstring].db.entity_by_features(constraints=constraints)
            lock.release()
            return entities
        return {}
    ...
    def get_random_slot_name(self, dstring):
        return self.SetObj.random.choice(self.ontologyManagers[dstring].ontology['requestable'])
    ...
    def _load_domains_ontology(self, domainString):
        ...
        if self.cfg.has_option('ontology_' + domainString, 'handler'):
            ontologyClass = self.cfg.get('ontology_' + domainString, 'handler')
        
        if ontologyClass is None:
            return FlatDomainOntology(domainString, self.cfg, self.OntologyUtils, self.SetObj)
        else:
            try:
