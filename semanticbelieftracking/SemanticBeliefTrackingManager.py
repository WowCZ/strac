...
class SemanticBeliefTracker(object):   
    ...
    ''' 
    def __init__(self,dstring,ontology):
...
class SemanticBeliefTrackingManager(object):
    ...
    def __init__(self, ontology, cfg, logger):
        self.ontology = ontology
        self.cfg = cfg
        self.logger = logger
        self.domainSemiBelieftrackers = dict.fromkeys(self.ontology.OntologyUtils.available_domains, None)
        ...
        for dstring in self.domainSemiBelieftrackers:
            if self.cfg.has_option("semibelief_" + dstring, "preload"):
                preload = self.cfg.getboolean("semibelief_" + dstring, "preload")
                if preload:
                    self.bootup(dstring)
    ...
    def bootup(self, domainString, previousDomainString=None):
        ...
        if self.domainSemiBelieftrackers[domainString] is not None:
            self.logger.warning('{} semi belief tracker is already loaded'.format(domainString))
        else:
    ...
    def _load_domains_semantic_belief_tracker(self, domainString):
        ...
        if not self.cfg.has_section('semibelief_' + domainString):
            if domainString not in self.SPECIAL_DOMAINS: # it is ok to load modular semantic belief tracker for special domains
                self.logger.debug("No semibelief section specified for domain {} - defaulting to modular semibelief".format(domainString))
        else:
            if self.cfg.has_option('semibelief_' + domainString, 'type'):
                semiBeliefType = self.cfg.get('semibelief_' + domainString, 'type')
                
                
        if semiBeliefType == "modularSemiBelief":
            from ModularSemanticBeliefTracker import ModularSemanticBeliefTracker
            self.domainSemiBelieftrackers[domainString] = ModularSemanticBeliefTracker(domainString, self.ontology, self.cfg, self.logger)
            ...
            try:
                ...
                self.logger.error('Invalid semantic belief tracking type "{}" for domain "{}"'.format(semiBeliefType, domainString))
