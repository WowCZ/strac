...
class BeliefTrackingManager(object):
    ...
    '''
    def __init__(self, ontology, logger, cfg):
        self.logger = logger
        self.ontology = ontology
        self.cfg = cfg
        self.domainBeliefTrackers = dict.fromkeys(self.ontology.OntologyUtils.available_domains, None)
        self.SPECIAL_DOMAINS = ['topicmanager','wikipedia','ood']
        self.CONDITIONAL_BELIEF = False
        if self.cfg.has_option("conditional","conditionalbeliefs"):
            self.CONDITIONAL_BELIEF = self.cfg.getboolean("conditional","conditionalbeliefs")
    ...
    def restart(self):
        ...
        if self.CONDITIONAL_BELIEF:
            self.prev_domain_constraints = dict.fromkeys(self.ontology.OntologyUtils.available_domains) # used to facilate cond. behaviour
    ...
    def _load_domains_belieftracker(self, domainString=None):
        ...
        if self.cfg.has_option('policy_'+domainString, 'belieftype'):
            belief_type = self.cfg.get('policy_'+domainString, 'belieftype')

        if belief_type == 'focus':
            from baseline import FocusTracker
            self.domainBeliefTrackers[domainString] = FocusTracker(domainString, self.ontology)
        elif belief_type == 'baseline':
            from baseline import BaselineTracker
            self.domainBeliefTrackers[domainString] = BaselineTracker(domainString, self.ontology)
...
#             beliefs = BeliefTrackerRules.RNNTracker()
            ...
            try:
                ...
                self.domainBeliefTrackers[domainString] = klass(domainString, self.ontology)
            except ImportError:
                self.logger.error('Invalid semantic belief tracking type "{}" for domain "{}"'.format(belief_type, domainString))
