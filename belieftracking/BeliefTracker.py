...
class BeliefTracker(object):
    ...
    '''
    def __init__(self, domainString, ontology):
...
#         self._print_belief()
        ...
        # print(threading.currentThread().getName() + '  Turn: ' + str(self.turn))
        # print(threading.currentThread().getName() + '  ID: ' + str(id(self)))
        self.turn += 1
...
#         self._printTopBeliefs()
    ...
    def get_conditional_constraints(self, prev_domain_constraints):
        ...
        constraints = dict.fromkeys(self.ontology.get_system_requestable_slots(self.domainString))
        informable_vals = self.ontology.get_informable_slots_and_values(self.domainString)
...
#         return pprint.pformat(BeliefTrackingUtils.simplify_belief(self.ontology.get_ontology(self.domainString),
#                                                                   self.prevbelief))
    ...
    def _convertHypToTurn(self, lastact, obs):
        ...
        if self.turn > 0:
            ...
            slastact = BeliefTrackingUtils._transformAct(slastact, {}, 
                                                         self.ontology.get_ontology(self.domainString), 
                                                         user=False)
        ...
        for (hyp, prob) in obs:
            ...
            hyp = BeliefTrackingUtils._transformAct(hyp, {}, self.ontology.get_ontology(self.domainString))
    ...
    def _init_belief(self, constraints=None):
        ...
        ''' 
        belief = {} 
        for slot in self.ontology.get_informable_slots_and_values(self.domainString):
            inform_slot_vals = self.ontology.get_informable_slot_values(self.domainString,slot)
            if slot not in self.ontology.get_system_requestable_slots(self.domainString):
        ...
        belief['method'] = dict.fromkeys(self.ontology.get_method(self.domainString), 0.0)
        belief['method']['none'] = 1.0
        belief['discourseAct'] = dict.fromkeys(self.ontology.get_discourseAct(self.domainString), 0.0)
        belief['discourseAct']['none'] = 1.0
        belief['requested'] = dict.fromkeys(self.ontology.get_requestable_slots(self.domainString), 0.0)
    ...
    def _updateMactFeat(self, last_feature, lastact):
        ...
        :return: None
        '''
        # print('hhhhhhhhhhh'+str(last_feature))
        ...
        for numAccepted in range(1,6):
            temp =  SummaryUtils.actionSpecificInformSummary(self.ontology, self.prevbelief, numAccepted, self.domainString)
