...
class TopicTrackingManager(object):
    ...
    def __init__(self, cfg, ontology):
        ...
        if cfg.has_option("topictracker","type"):
            self.topic_tracker_type = cfg.get("topictracker","type")
        if self.topic_tracker_type == "keyword":
            from RuleTopicTrackers import KeywordSpottingTopicTracker
            self.tt_model = KeywordSpottingTopicTracker(cfg)
        ...
        if cfg.has_option("GENERAL","singledomain"):
            self.USE_SINGLE_DOMAIN = cfg.getboolean("GENERAL","singledomain")
        if self.USE_SINGLE_DOMAIN:
            # will now be restricted to a single doman system:
            # Below cfg.get is safe -- all hubs have already checked config has this section
            domain = cfg.get("GENERAL","domains")  
            if "," in domain:
                logger.error("It's confusing to specify more than 1 domain if you want to use single domain system")
            if domain not in ontology.OntologyUtils.available_domains:
            ...
            self.domainsLastAct = dict.fromkeys(ontology.OntologyUtils.available_domains, None)  # NB: is for multidomain only
