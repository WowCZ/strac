...
def _add_context_to_user_act(sys_act, hyps, active_domain, ontology):
    '''
    ...
    for hyp in hyps:
        if hyp[0] in ['affirm()','negate()']:
            user_act,prob = hyp
            user_act = _convert_yes_no(sys_act, user_act, active_domain, ontology)  
...
def _convert_yes_no(sys_act, user_act, active_domain, ontology):
    '''
    ...
    if ontology.OntologyUtils.BINARY_SLOTS[active_domain] is None:
        ontology.updateBinarySlots(active_domain)
    ...
    if dact.act in ['request', 'confirm']: 
        for item in dact.items:         
            ...
            if dact.act == 'request' and slot not in ontology.OntologyUtils.BINARY_SLOTS[active_domain]:
