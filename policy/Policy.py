...
class Policy(object):
    ...
    '''
    def __init__(self, domainString, ontology, cfg, logger, SetObj, learning=False, specialDomain=False): 
        """
        ...
        if cfg.has_option('policy', 'startwithhello'):
            self.startwithhello = cfg.getboolean('policy', 'startwithhello')
        if cfg.has_option('policy_'+domainString, 'startwithhello'):
            self.startwithhello = cfg.getboolean('policy_'+domainString, 'startwithhello')
        
        self.useconfreq = False
        if cfg.has_option('policy', 'useconfreq'):
            self.useconfreq = cfg.getboolean('policy', 'useconfreq')
        if cfg.has_option('policy_'+domainString, 'useconfreq'):
            self.useconfreq = cfg.getboolean('policy_'+domainString, 'useconfreq')
        ...
        if cfg.has_option("policy", "usestack"):
            self.USE_STACK = cfg.getboolean("policy", "usestack")
        if cfg.has_option("policy_"+domainString, "usestack"):
            self.USE_STACK = cfg.getboolean("policy_"+domainString, "usestack")
        if self.USE_STACK:
            ...
            if cfg.has_option("policy", "processstack"):
                self.PROCESS_EPISODE_STACK = cfg.getint("policy_"+domainString, "processstack")
            if cfg.has_option("policy", "processstack"):
                self.PROCESS_EPISODE_STACK = cfg.getint("policy_"+domainString, "processstack")
        ...
        self.actions = SummaryAction.SummaryAction(domainString, self.ontology, self.SetObj, empty, self.useconfreq)
        # Total number of system actions.
        self.numActions = len(self.actions.action_names)
        
        self.episodes = dict.fromkeys(self.ontology.OntologyUtils.available_domains, None)
