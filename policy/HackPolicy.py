...
class DQNPolicy(Policy.Policy):
    ...
    def __init__(self, in_policy_file, out_policy_file, ontology, cfg, logger, SetObj, domainString='CamRestaurants', is_training=False):
        super(DQNPolicy, self).__init__(domainString, ontology, cfg, logger, SetObj, is_training)
        ...
        if cfg.has_option('dqnpolicy', 'learning_rate'):
            self.learning = cfg.get("policy", "learning")
    ...
    def record(self, reward, domainInControl=None, weight=None, state=None, action=None):
        ...
        # print('Learning is '+str(self.learning))
        # exit(0)
    ...
    def _sample_and_updateQ(self):
        ...
        return s_batch, a_batch_one_hot, reshaped_yi
    ...
    def train(self):
        ...
        if self.samplecount >= self.minibatch_size * 10 and globalEpisodeCount % self.training_frequency == 0:
            ...
            # print(Settings.global_policysaver)
            # print(len(Settings.global_policysaver))
            assert len(Settings.global_policysaver) == Settings.global_threadsnum
            # self.dqn.reset_noise()
            total_batch_size = 0
            for k, v in Settings.global_policysaver.items():
                s_batch, a_batch_one_hot, reshaped_yi = Settings.global_hackpolicysaver[k]._sample_and_updateQ()
                grad, batch_size = Settings.global_policysaver[k].train(s_batch, a_batch_one_hot, reshaped_yi)
