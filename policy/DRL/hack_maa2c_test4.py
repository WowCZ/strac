    ...
    def __init__(self, domain_string, hidden_layers, local_hidden_units, local_dropouts,
        ...
        if self.domain_string == 'Laptops11':
            ...
            if self.state_feature == 'vanilla':
                ...
                self.global_dimension = [(275, 352), ]
                self.input_dimension = 352
                self.global_input_dimension = 352 - 275
            ...
            if self.state_feature == 'vanilla':
                ...
                self.global_dimension = [(150, 227), ]
                self.input_dimension = 227
                self.global_input_dimension = 227 - 150
            ...
            if self.state_feature == 'vanilla':
                ...
                self.global_dimension = [(75, 152), ]
                self.input_dimension = 152
                self.global_input_dimension = 152 - 75
        else:
            raise ValueError
        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    self.output_trans_local_slotQ1 = NoisyDense(in_units=slot_dim, units=64, activation='relu')
                    self.output_trans_local_slotQ2 = NoisyDense(in_units=64, units=1)
                    self.output_trans_local_slotP = NoisyDense(in_units=self.local_hidden_units[-1], units=1, use_bias=False)
                    self.output_trans_global_slotQ1 = NoisyDense(in_units=self.global_input_dimension, units=64, activation='relu')
                    self.output_trans_global_slotQ2 = NoisyDense(in_units=64, units=1)
        ...
        if self.share_last_layer is False:
            ...
            for i in range(len(self.slots) + 1):
                if self.use_dueling is False:
                    ...
                    if self.dueling_share_last:
                        if i < len(self.slots):
                            cur_slotq = self.output_trans_local_slotQ1.forward(sorted_inputs[i], training=training)
                            cur_slotq = self.output_trans_local_slotQ2.forward(cur_slotq, training=training)
                            ...
                            cur_slotq = self.output_trans_global_slotQ1.forward(sorted_inputs[i], training=training)
                            cur_slotq = self.output_trans_global_slotQ2.forward(cur_slotq, training=training)
            ...
            value = nd.sum(batch_slot_prob * batch_slot_slotq, axis=1)
...
class A2CNetwork(object):
    ...
    def __init__(self, state_dim, action_dim, learning_rate, tau,
        ...
        # self.actor_trainer = gl.Trainer(params=self.actorcritic.collect_params(), optimizer='adam',
        #                           optimizer_params=dict(learning_rate=self.actor_learning_rate, wd=weight_regularizer))

        # self.critic_trainer = gl.Trainer(params=self.actorcritic.collect_params(), optimizer='adam',
        #                           optimizer_params=dict(learning_rate=self.critic_learning_rate, wd=weight_regularizer))
    ...
    def train(self, s_batch, a_batch_one_hot, V_trace, advantage):
        ...
        with mx.autograd.record():
            loss_vec = []
            probs, values = self.actorcritic.forward(s_batch, loss_vec)
            loss = 0.
            ...
            actorloss = -nd.sum(logprob*advantage_batch, axis=0) - 0.1*entropy
