...
class MATransfer(gl.nn.Block):
    def __init__(self, slots, local_in_units, local_units, local_dropout,
        ...
        self.topo_dropout = True
        self.topo_dropout_rate = 0.2
    ...
    def forward(self, inputs, loss=None, topo_dropout=True):
        ...
        if topo_dropout:
            for i in range(self.slots+1):
                for j in range(self.slots+1):
                    rand = random.random()
                    if rand < self.topo_dropout_rate:
                        comm_rate[i][j] = 0
...
class MultiAgentNetwork(gl.nn.Block):
    ...
    def forward(self, input_vec, loss=None, topo_dropout=True):
        ...
        for i in range(self.hidden_layers - 1):
            if self.recurrent_mode is False:
                ...
                layer.append(self.ma_trans[i](layer[i], loss, topo_dropout))
            else:
                layer.append(self.ma_trans(layer[i], loss, topo_dropout))
...
class DeepQNetwork(object):
    ...
    def predict(self, inputs):
        ...
        return self.qnet.forward(nd.array(inputs,ctx=CTX), topo_dropout=False).asnumpy()

    def predict_target(self, inputs):
        if self.target_explore:
            with mx.autograd.train_mode():
                return self.target.forward(nd.array(inputs, ctx=CTX), topo_dropout=False).asnumpy()
        else:
