...
class MATransfer(gl.nn.Block):
    ...
    def forward(self, inputs, loss=None, training=True, commtype='average'):
        ...
        results.append(self.global_trans.forward(inputs[-1]))
            ...
            if self.topo_learning_mode:
                ...
                if commtype == 'average':
                    for i in range(self.slots):
                        ...
                        for j in range(self.slots):
                            if j != i:
                                tmp = tmp + self.local2local_share_comm.forward(inputs[j])
                        tmp = tmp + self.global2local_comm.forward(inputs[-1])
                        results[i] = results[i] + (tmp / float(self.slots))

                    tmp = nd.zeros_like(results[-1])
                    for i in range(self.slots):
                        tmp = tmp + self.local2global_comm.forward(inputs[i])
                    results[-1] = results[-1] + (tmp / float(self.slots))
                    ...
                    for i in range(self.slots):
                        ...
                        for j in range(self.slots):
                            if j != i:
                                tmp.append(self.local2local_share_comm.forward(inputs[j]))
                        tmp.append(self.global2local_comm.forward(inputs[-1]))
                    ...
                    tmp = []
                    for i in range(self.slots):
                        tmp.append(self.local2global_comm.forward(inputs[i]))
                    for k in range(len(tmp)):
                        tmp[k] = tmp[k].reshape((tmp[k].shape[0], 1, tmp[k].shape[1]))
...
class MultiAgentNetwork(gl.nn.Block):
        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    self.output_trans_local_slotQ = gl.nn.Dense(in_units=self.local_hidden_units[-1], units=1, use_bias=False)
                    self.output_trans_local_slotP = gl.nn.Dense(in_units=self.local_hidden_units[-1], units=1)
                    self.output_trans_global_slotQ = gl.nn.Dense(in_units=self.global_hidden_units[-1], units=1)
                    self.output_trans_global_slotP = gl.nn.Dense(in_units=self.global_hidden_units[-1], units=1)
                    self.output_trans_local_valueP = gl.nn.Dense(in_units=self.local_hidden_units[-1], units=3)
                    self.output_trans_global_valueP = gl.nn.Dense(in_units=self.global_hidden_units[-1], units=7)
                    if self.shared_last_layer_use_bias:
    ...
    def forward(self, input_vec, loss=None, training=True):
        ...
        if (not self.sort_input_vec) and self.state_feature != 'dip':
            ...
            layer.append(self.input_trans.forward(sorted_inputs, loss))
        elif self.sort_input_vec:
            sorted_inputs = []
            for slot in self.slots:
        ...
        if self.share_last_layer is False:
            ...
            for i in range(len(self.slots) + 1):
                if self.use_dueling is False:
                    ...
                    if i < len(self.slots):
                        cur_slotv_prob = self.output_trans_local_valueP.forward(layer[-1][i])
                        # cur_slotv_prob = nd.softmax(cur_slotv_prob)
                    else:
                        cur_slotv_prob = self.output_trans_global_valueP.forward(layer[-1][i])
                        # cur_slotv_prob = nd.softmax(cur_slotv_prob)
                    ...
                    if self.dueling_share_last:
                        if i < len(self.slots):
                            cur_slotq = self.output_trans_local_slotQ.forward(layer[-1][i])
                            cur_slot_prob = self.output_trans_local_slotP.forward(layer[-1][i]).reshape(-1,1)
                            ...
                            cur_slotq = self.output_trans_global_slotQ.forward(layer[-1][i])
                            cur_slot_prob = self.output_trans_global_slotP.forward(layer[-1][i]).reshape(-1,1)
                            # cur_slotv_prob = cur_slotv_prob*cur_slot_prob
                            # cur_slot_prob = nd.softmax(cur_slot_prob)
