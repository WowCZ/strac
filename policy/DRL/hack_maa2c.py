        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    self.output_trans_local_slotQ = NoisyDense(in_units=self.local_hidden_units[-1], units=1, use_bias=False)
...
class A2CNetwork(object):
    ...
    def train(self, s_batch, a_batch_one_hot, V_trace, advantage):
        ...
        action_bp_rate = 1 - np.array(action_stats)/float(batch_size)
        ...
        with mx.autograd.record():
            ...
            loss = actorloss + 0.1*criticloss + 0.001*entropy_loss
        loss.backward()
