        ...
        with self.name_scope():
            ...
            if self.non_local_mode is False:
                if self.message_embedding:
                    self.local_trans = NoisyDense(in_units=local_in_units, units=local_units, activation=activation)
                    self.global_trans = NoisyDense(in_units=global_in_units, units=global_units, activation=activation)
                    if self.use_comm:
                        msg_dim = 16
                        emb_dim = 64
                        self.local2global_msg_encode = NoisyDense(in_units=local_in_units, units=msg_dim)
                        self.local2local_msg_encode = NoisyDense(in_units=local_in_units, units=msg_dim)
                        self.global2local_msg_encode = NoisyDense(in_units=global_in_units, units=msg_dim)

                        self.local2global_embedding = NoisyDense(in_units=msg_dim, units=emb_dim)
                        self.local2local_embedding = NoisyDense(in_units=msg_dim, units=emb_dim)
                        self.global2local_embedding = NoisyDense(in_units=msg_dim, units=emb_dim)

                        self.local2global_extract = NoisyDense(in_units=emb_dim, units=global_units)
                        self.local2local_extract = NoisyDense(in_units=emb_dim, units=local_units)
                        self.global2local_extract = NoisyDense(in_units=emb_dim, units=local_units)
                else:
                    self.local_share_trans = NoisyDense(in_units=local_in_units, units=local_units, activation=activation)
                    self.global_trans = NoisyDense(in_units=global_in_units, units=global_units, activation=activation)
                    if self.use_comm:
                        if self.slots_comm:
                            self.local2local_share_comm = NoisyDense(
                                in_units=local_in_units, units=local_units, activation=activation)
                        self.local2global_comm = NoisyDense(in_units=local_in_units, units=global_units, activation=activation)
                        self.global2local_comm = NoisyDense(in_units=global_in_units, units=local_units, activation=activation)
                ...
                self.global_trans = NoisyDense(in_units=global_in_units, units=global_units, activation=activation)
                mid_units = 64
                self.g_local2local = NoisyDense(in_units=local_in_units, units=local_units, use_bias=False)
                self.g_global2local = NoisyDense(in_units=global_in_units, units=local_units, use_bias=False)
                self.g_local2global = NoisyDense(in_units=local_in_units, units=global_units, use_bias=False)
                if self.slots_comm:
                    self.f_emit_local2local = NoisyDense(in_units=local_in_units, units=mid_units, use_bias=False)
                self.f_emit_local2global = NoisyDense(in_units=local_in_units, units=mid_units, use_bias=False)
                self.f_emit_global2local = NoisyDense(in_units=global_in_units, units=mid_units, use_bias=False)
                self.f_rec_local = NoisyDense(in_units=local_in_units, units=mid_units, use_bias=False)
                self.f_rec_global = NoisyDense(in_units=global_in_units, units=mid_units, use_bias=False)

            if self.block_mode:
                self.yz_weight_local = NoisyDense(in_units=local_in_units, units=local_units, use_bias=False)
                self.yz_weight_global = NoisyDense(in_units=global_in_units, units=global_units, use_bias=False)

    def forward_non_local(self, inputs, loss, training):
        results = []
        for i in range(self.slots):
            results.append(self.local_trans.forward(inputs[i], training=training))
        ...
        if self.use_comm:
            ...
            for i in range(self.slots):
                ...
                for j in range(self.slots):
                    if i != j:
                        f[i][j] = nd.sum(self.f_rec_local.forward(inputs[i], training=training) * self.f_emit_local2local(inputs[j], training=training), axis=1)
                        f[i][j] = nd.exp(f[i][j]).reshape((f[i][j].shape[0], 1))
                        f[i][j] = f[i][j] * comm_rate[j][i]
                        if norm_fac is None:
                            norm_fac = nd.zeros_like(f[i][j])
                        norm_fac = norm_fac + f[i][j]
                f[i][-1] = nd.sum(self.f_rec_local.forward(inputs[i], training=training) * self.f_emit_global2local(inputs[-1], training=training), axis=1)
            ...
            norm_fac = None
            for i in range(self.slots):
                f[-1][i] = nd.sum(self.f_rec_global.forward(inputs[-1], training=training) * self.f_emit_local2global(inputs[i], training=training), axis=1)
        ...
        if self.block_mode:
            ...
            for i in range(self.slots):
                results[i] = self.yz_weight_local.forward(results[i], training=training) + inputs[i]
            results[-1] = self.yz_weight_global.forward(results[-1], training=training) + inputs[-1]
    ...
    def forward_message_embedding(self, inputs, loss, training):
        ...
        if self.use_comm:
            for i in range(self.slots):
                ...
                for j in range(self.slots):
                    msg = nd.softmax(self.local2local_msg_encode.forward(inputs[j], training=training))
                    tmp = tmp + self.local2local_extract(self.local2local_embedding(msg, training=training), training=training)
                msg = nd.softmax(self.global2local_msg_encode.forward(inputs[-1], training=training))
                tmp = tmp + self.global2local_extract(self.global2local_embedding.forward(msg, training=training), training=training)
                results[i] = results[i] + (tmp / float(self.slots))
            tmp = nd.zeros_like(results[-1])
            for i in range(self.slots):
                msg = nd.softmax(self.local2global_msg_encode.forward(inputs[i], training=training))
                tmp = tmp + self.local2global_extract(self.local2global_embedding.forward(msg, training=training), training=training)
        ...
        if self.use_comm:
            if self.topo_learning_mode:
                ...
                for i in range(self.slots):
                    ...
                    for j in range(self.slots):
                        if i != j:
                            tmp = tmp + self.local2local_share_comm.forward(inputs[j], training=training) * comm_rate[j][i]
                            norm = norm + comm_rate[j][i]
                    # results[i] = results[i] + self.global2local_comm(inputs[-1]) * comm_rate[-1][i]
                    tmp = tmp + self.global2local_comm.forward(inputs[-1], training=training) * comm_rate[-1][i]
                ...
                norm = nd.zeros_like(comm_rate[0][0])
                for j in range(self.slots):
                    tmp = tmp + self.local2global_comm.forward(inputs[j], training=training) * comm_rate[j][-1]
...
class MultiAgentNetwork(gl.nn.Block):
    def __init__(self, domain_string, hidden_layers, local_hidden_units, local_dropouts,
                 ...
                 use_dueling, dueling_share_last, message_embedding, state_feature, shared_last_layer_use_bias, atoms, **kwargs):
            ...
            if (self.sort_input_vec is False) and (self.state_feature != 'dip'):
                ...
                for slot in self.slots:
                    ...
                    self.input_trans[slot] = \
                        NoisyDense(in_units=in_units, units=self.local_hidden_units[0], activation='relu')
                    self.register_child(self.input_trans[slot])
                self.input_trans['global'] = \
                    NoisyDense(in_units=self.global_input_dimension,
            ...
            if self.use_dueling is False:
                if self.share_last_layer is False:
                    ...
                    for i in range(len(self.slots)):
                        self.output_trans.append(NoisyDense(in_units=self.local_hidden_units[-1], units=3))
                        self.register_child(self.output_trans[-1])
                    self.output_trans.append(NoisyDense(in_units=self.global_hidden_units[-1], units=7))
                    self.register_child(self.output_trans[-1])
                else:
                    self.output_trans_local = NoisyDense(in_units=self.local_hidden_units[-1], units=3, use_bias=False)
                    ...
                    self.output_trans_global = NoisyDense(in_units=self.global_hidden_units[-1], units=7)
            else:
                assert self.sort_input_vec or self.state_feature == 'dip'
                if self.dueling_share_last:
                    self.output_trans_local_value = NoisyDense(in_units=self.local_hidden_units[-1], units=self.atoms, use_bias=False)
                    self.output_trans_global_value = NoisyDense(in_units=self.global_hidden_units[-1], units=self.atoms)
                    if self.shared_last_layer_use_bias:
                        self.value_bias_local = self.params.get(name='value_bias_local', shape=(len(self.slots), ))
                else:
                    self.output_trans_value = []
                    for i in range(len(self.slots)):
                        self.output_trans_value.append(NoisyDense(in_units=self.local_hidden_units[-1], units=self.atoms))
                        self.register_child(self.output_trans_value[-1])
                    self.output_trans_value.append(NoisyDense(in_units=self.global_hidden_units[-1], units=self.atoms))
                    self.register_child(self.output_trans_value[-1])

                slot_dim = 22 if self.sort_input_vec else 25
                self.output_trans_local_advantage1 = NoisyDense(in_units=slot_dim, units=64, activation='relu')
                self.output_trans_local_advantage2 = NoisyDense(in_units=64, units=3*self.atoms)

                self.output_trans_global_advantage1 = NoisyDense(in_units=self.global_input_dimension, units=100, activation='relu')
                self.output_trans_global_advantage2 = NoisyDense(in_units=100, units=7*self.atoms)
    ...
    def forward(self, input_vec, loss=None, training=True):
        ...
        if (not self.sort_input_vec) and self.state_feature != 'dip':
            layer.append([])
            for slot in self.slots:
                layer[0].append(self.input_trans[slot].forward(inputs[slot], training=training))
            layer[0].append(self.input_trans['global'].forward(inputs['global'], training=training))
        elif self.state_feature == 'dip':
            sorted_inputs = []
            for slot in self.slots:
        ...
        if self.share_last_layer is False:
            ...
            for i in range(len(self.slots) + 1):
                if self.use_dueling is False:
                    outputs.append(self.output_trans[i](layer[-1][i], training=training))
                else:
                    if i < len(self.slots):
                    ...
                    batchsize = cur_value.shape[0]
                    v, a = cur_value.reshape((batchsize, 1, self.atoms)), tmp_adv.reshape((batchsize, -1, self.atoms))

                    outputs.append(v + a - a.mean(axis=1, keepdims=True))
        else:
            print('Bug in hack_marainbow!')
            exit(0)

        # if log:
        #     p = nd.log_softmax(nd.concat(*outputs, dim=1), dim=2)
        # else:
        p = nd.softmax(nd.concat(*outputs, dim=1), axis=2)
...
class DeepRBQNetwork(object):
    """
    Input to the network is the state and action, output is Q(s,a).
    """

    def __init__(self, state_dim, action_dim, atoms, learning_rate, tau,
    ...
    def train(self, inputs, action, targetdis):
        inputs = copy.deepcopy(inputs)
        action = copy.deepcopy(action)
        targetdis = copy.deepcopy(targetdis)

        inputs = nd.array(inputs, ctx=CTX)
        action = nd.broadcast_axis(nd.array(action, ctx=CTX), axis=2, size=self.atoms)
        targetdis = nd.array(targetdis, ctx=CTX)
        ...
        with mx.autograd.record():
            ...
            for element in loss_vec:
                loss = loss + element
            dis_est = nd.sum(data=outputs * action, axis=1)
            loss = -nd.sum(nd.sum(targetdis*nd.log(dis_est), axis=1))
            # print(dis_est.shape)
            # print(loss.shape)
            # exit(0)
            # print loss
    ...
    def reset_noise(self):
        for name, child in self.qnet._children.items():
            if name.find('drop') < 0:
                # print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
                # print(name)
                child.reset_noise()
    ...
    def reset_target_noise(self):
        for name, child in self.target._children.items():
            if name.find('drop') < 0:
                child.reset_noise()
            else:
                continue

    def predict(self, inputs, support):
        assert mx.autograd.is_training() is False
        pns = self.get_distribution(inputs)
        tmp = np.vstack([np.expand_dims(support, 0)]*pns.shape[1])
        support_exp = np.vstack([np.expand_dims(tmp, 0)]*pns.shape[0])
        action_q = np.sum(support_exp * pns, axis=2)
        # print(support_exp)
        # print(support_exp.shape)
        # print(action_q.shape)
        # exit(0)
        return action_q

    def predict_target(self, inputs, support):
        pns = self.get_target_distribution(inputs)
        tmp = np.vstack([np.expand_dims(support, 0)]*pns.shape[1])
        support_exp = np.vstack([np.expand_dims(tmp, 0)]*pns.shape[0])
        action_q = np.sum(support_exp * pns, axis=2)
        return action_q
    ...
    def get_target_distribution(self, inputs):
