...
class MultiAgentNetwork(gl.nn.Block):
    def __init__(self, domain_string, hidden_layers, local_hidden_units, local_dropouts,
        ...
        with self.name_scope():
            ...
            if self.recurrent_mode is False:
                ...
                self.register_child(self.ma_trans)
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    self.output_trans_local_slotP = NoisyDense(in_units=self.local_hidden_units[-1], units=3)
                    self.output_trans_global_slotP = NoisyDense(in_units=self.global_hidden_units[-1], units=7)
                    # self.output_trans_value = NoisyDense(in_units=self.global_hidden_units[-1]+self.local_hidden_units[-1], units=1)
                    if self.shared_last_layer_use_bias:
                        self.value_bias_local = self.params.get(name='value_bias_local', shape=(len(self.slots), ))
                else:
                    self.output_trans_value = []
                    for i in range(len(self.slots)):
                        self.output_trans_value.append(NoisyDense(in_units=self.local_hidden_units[-1], units=1))
                        self.register_child(self.output_trans_value[-1])
                    self.output_trans_value.append(NoisyDense(in_units=self.global_hidden_units[-1], units=1))
                    self.register_child(self.output_trans_value[-1])

                slot_dim = 22 if self.sort_input_vec else 25
                self.output_trans_local_value = NoisyDense(in_units=slot_dim, units=64, activation='relu')
                self.output_trans_global_value = NoisyDense(in_units=self.global_input_dimension, units=64, activation='relu')
                self.output_trans_value = NoisyDense(in_units=64, units=1)
    ...
    def forward(self, input_vec, loss=None, training=True):
        ...
        if self.share_last_layer is False:
            ...
            for i in range(len(self.slots) + 1):
                if self.use_dueling is False:
                    ...
                    if i < len(self.slots):
                        ...
                        cur_slot_v = self.output_trans_local_value.forward(sorted_inputs[i], training=training)
                    else:
                        cur_slot_prob = self.output_trans_global_slotP.forward(layer[-1][i], training=training)
                        cur_slot_v = self.output_trans_global_value.forward(sorted_inputs[i], training=training)
            ...
            batch_value = nd.squeeze(self.output_trans_value.forward(nd.add_n(*slot_hidden_value), training=training))
...
class A2CNetwork(object):
    ...
    def __init__(self, state_dim, action_dim, learning_rate, tau,
        ...
        self.actortrainer = gl.Trainer(params=self.actorcritic.collect_params(), optimizer='adam',
                                  optimizer_params=dict(learning_rate=self.learning_rate, clip_gradient=10))

        self.critictrainer = gl.Trainer(params=self.actorcritic.collect_params(), optimizer='adam',
    ...
    def train(self, s_batch, a_batch_one_hot, V_trace, advantage):
        ...
        with mx.autograd.record():
            loss_vec = []
            probs, _ = self.actorcritic.forward(s_batch, loss_vec)
            logprob = nd.log(nd.sum(data=probs * a_batch_one_hot, axis=1))
            entropy = -nd.sum(nd.sum(data=probs*nd.log(probs), axis=1), axis=0)
            actorloss = -nd.sum(logprob*advantage_batch, axis=0) - entropy
        ...
        with mx.autograd.record():
            loss_vec = []
            _, values = self.actorcritic.forward(s_batch, loss_vec)
    ...
    def predict_action_value(self, inputs):
        ...
        prob, value = self.actorcritic.forward(nd.array(inputs, ctx=CTX))
        prob = prob.asnumpy()
        value = value.asnumpy()
        # print(prob)
        return prob, value
    ...
    def predict_value(self, inputs):
        self.reset_noise()
        assert mx.autograd.is_training() is False
        _, value = self.actorcritic.forward(nd.array(inputs, ctx=CTX))
        value = value.asnumpy()
        return value
    ...
    def getPolicy(self, inputs):
        self.reset_noise()
        assert mx.autograd.is_training() is False
        prob, _ = self.actorcritic.forward(nd.array(inputs, ctx=CTX))
        prob = prob.asnumpy()
        return prob
    ...
    def load_trainer(self, load_filename):
        self.actortrainer.step(1, ignore_stale_grad=True)
        self.actortrainer.load_states(fname=load_filename + '_' + self.domain_string + '_actortrainer')

        self.critictrainer.step(1, ignore_stale_grad=True)
        self.critictrainer.load_states(fname=load_filename + '_' + self.domain_string + '_critictrainer')
    ...
    def save_trainer(self, save_filename):
        ...
        self.actortrainer.step(1, ignore_stale_grad=True)
        self.actortrainer.save_states(fname=save_filename +'_' + self.domain_string +'_actortrainer')
        print('save actor trainer as ' + save_filename+'_'+self.domain_string+'_actortrainer')

        self.critictrainer.step(1, ignore_stale_grad=True)
        self.critictrainer.save_states(fname=save_filename +'_' + self.domain_string +'_critictrainer')
        print('save critic trainer as ' + save_filename+'_'+self.domain_string+'_critictrainer')
