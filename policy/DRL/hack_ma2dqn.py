...
class MATransfer(gl.nn.Block):
        ...
        with self.name_scope():
            if self.topo_learning_mode:
                self.topo = self.params.get(
                    name='topo_mat', shape=(self.slots, self.slots), init=mx.init.Constant(2.))
    ...
    def forward_non_local(self, inputs, loss):
        ...
        comm_rate = nd.ones(shape=(self.slots, self.slots))
        if self.use_comm and self.topo_learning_mode:
            ...
            u_vec = nd.random_uniform(low=1e-5, high=1. - 1e-5, shape=(self.slots, self.slots))
        ...
        if self.use_comm:
            if self.topo_learning_mode:
                ...
                for i in range(self.slots):
                    ...
                    results[i] = results[i] + (tmp / float(self.slots-1))
...
class MultiAgentNetwork(gl.nn.Block):
        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    ...
                    self.output_trans_global_value = gl.nn.Dense(in_units=self.global_hidden_units[-1], units=2)
                    if self.shared_last_layer_use_bias:
                    ...
                    self.output_trans_value.append(gl.nn.Dense(in_units=self.global_hidden_units[-1], units=2))
                    self.register_child(self.output_trans_value[-1])
    ...
    def forward(self, input_vec, loss=None):
        ...
        if self.share_last_layer is False:
            ...
            outputs = []
            cur_value_gl = self.output_trans_global_value(layer[-1][-1])
            for i in range(len(self.slots) + 1):
                if self.use_dueling is False:
                    ...
                    if self.dueling_share_last:
                        if i < len(self.slots):
                            cur_value_l = self.output_trans_local_value(layer[-1][i])
                            if self.shared_last_layer_use_bias:
                                cur_value_l = cur_value_l + nd.slice(self.value_bias_local.data(), begin=(i, ), end=(i + 1, ))
                        # else:
                        #     cur_value = self.output_trans_global_value(layer[-1][i])
                    else:
                        if i < len(self.slots):
                            cur_value_l = self.output_trans_value[i](layer[-1][i])

                    # print cur_value_gl[:,0]
                    # print cur_value_gl[:,0].shape
                    # print '+++++++++++++++++++++++++++++++++++++++++++'

                    # print cur_value_l
                    # print cur_value_l.shape
                    # print '+++++++++++++++++++++++++++++++++++++++++++'
                    ...
                    # print tmp_adv.mean(axis=1).reshape(
                    #             (tmp_adv.shape[0], 1)).broadcast_axes(axis=1, size=tmp_adv.shape[1])
                    # print tmp_adv.mean(axis=1).reshape(
                    #             (tmp_adv.shape[0], 1)).broadcast_axes(axis=1, size=tmp_adv.shape[1]).shape
                    # print '+++++++++++++++++++++++++++++++++++++++++++'

                    if i < len(self.slots):
                        outputs.append(
                            cur_value_gl[:,0].reshape((-1,1)) + cur_value_l +
                            tmp_adv - tmp_adv.mean(axis=1).reshape(
                                (tmp_adv.shape[0], 1)).broadcast_axes(axis=1, size=tmp_adv.shape[1]))
                    else:
                        outputs.append(
                            cur_value_gl[:,1].reshape((-1,1)) +
                            tmp_adv - tmp_adv.mean(axis=1).reshape(
                                (tmp_adv.shape[0], 1)).broadcast_axes(axis=1, size=tmp_adv.shape[1]))
        else:
            outputs = []
