...
class MultiAgentNetwork(gl.nn.Block):
        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                if self.dueling_share_last:
                    ...
                    self.attention_layer = NoisyDense(in_units=self.global_hidden_units[-1]+self.local_hidden_units[-1], units=1)
                    self.private_w = self.params.get('private_w', shape=(1,), init=mx.init.One())
                    self.private_b = self.params.get('private_b', shape=(1,), init=mx.init.Zero())
                    self.private_w.initialize()
                    self.private_b.initialize()
                    if self.shared_last_layer_use_bias:
    ...
    def forward(self, input_vec, pop_art_hyper=None, loss=None, training=True):
        ...
        if self.share_last_layer is False:
            ...
            batch_value = nd.squeeze(self.output_trans_value.forward(slots_concat, training=training))
