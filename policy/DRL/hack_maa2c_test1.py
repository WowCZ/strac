    ...
    def __init__(self, domain_string, hidden_layers, local_hidden_units, local_dropouts,
        ...
        with self.name_scope():
            ...
            if self.use_dueling is False:
                ...
                self.output_trans_local_value = gl.nn.Dense(in_units=slot_dim, units=64, activation='relu')
                self.output_trans_global_value = gl.nn.Dense(in_units=self.global_input_dimension, units=64, activation='relu')
                self.output_trans_value = gl.nn.Dense(in_units=64, units=1)
