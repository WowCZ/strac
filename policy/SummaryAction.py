...
class SummaryAction(object):
    ...
    '''
    def __init__(self, domainString, ontology, SetObj, empty=False, confreq=False):
        '''
        ...
        cfg = SetObj.config
        if cfg.has_option("summaryacts", "informmask"):
            self.inform_mask = cfg.getboolean('summaryacts', 'informmask')
        self.inform_count_accepted = 4
        if cfg.has_option("summaryacts", "informcountaccepted"):
            self.inform_count_accepted = cfg.getint('summaryacts', 'informcountaccepted')
        elif cfg.has_option("goalgenerator", "maxconstraints"):
            self.inform_count_accepted = cfg.getint('goalgenerator', 'maxconstraints') + 1
        self.request_mask = True
        if cfg.has_option("summaryacts", "requestmask"):
            self.request_mask = cfg.getboolean('summaryacts', 'requestmask')
        self.bye_mask = True
        if cfg.has_option("summaryacts", "byemask"):
            self.request_mask = cfg.getboolean('summaryacts', 'byemask')

        print(self.ontology)
        # exit(0)

        if not empty:
            for slot in self.ontology.get_system_requestable_slots(domainString):
                self.action_names.append("request_" + slot)
                self.action_names.append("confirm_" + slot)
                self.action_names.append("select_" + slot)
                if confreq:
                    for slot2 in self.ontology.get_system_requestable_slots(domainString):
    ...
    def Convert(self, belief, action, lastSystemAction):
        ...
        self._array_slot_summary = SummaryUtils.arraySlotSummary(self.ontology, belief, self.domainString)
        self._global_summary = SummaryUtils.globalSummary(self.ontology, belief, self.domainString)
    ...
    def getNonExecutable(self, belief, lastSystemAction):
        ...
        array_slot_summary = SummaryUtils.arraySlotSummary(self.ontology, belief, self.domainString)
        global_summary = SummaryUtils.globalSummary(self.ontology, belief, self.domainString)
        ...
        for action in self.action_names:
            ...
            if action == "inform":
                acceptance_list = SummaryUtils.getTopBeliefs(self.ontology, belief, domainString=self.domainString)
                discriminable = SummaryUtils.acceptanceListCanBeDiscriminated(self.ontology, acceptance_list,
                                                                                                     self.domainString)
    ...
    def getInformByConstraints(self, belief):
        accepted_values = SummaryUtils.getTopBeliefs(self.ontology, belief, domainString=self.domainString)
        constraints = SummaryUtils.get_constraints(accepted_values)
        return SummaryUtils.getInformByConstraints(self.ontology, constraints, self.domainString, belief['features']['lastInformedVenue'], self.SetObj)
    ...
    def getInformByName(self, belief):
        ...
        return SummaryUtils.getInformRequestedSlots(self.ontology, requested_slots, name, self.domainString, self.SetObj)

    def getInformAlternatives(self, belief):
        self.alternatives_requested = False
        informedVenueSinceNone = set(belief['features']['informedVenueSinceNone'])
        accepted_values = SummaryUtils.getTopBeliefs(self.ontology, belief, domainString=self.domainString)
        return SummaryUtils.getInformAlternativeEntities(self.ontology, accepted_values, informedVenueSinceNone, self.domainString, self.SetObj)
