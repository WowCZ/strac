...
def getInformExactEntity(ontology, acceptanceList, numAccepted, domainString):
    ...
    result = ontology.entity_by_features(domainString, acceptedValues)
    if len(result) == 0:
...
def _getInformRequestedSlots(ontology, acceptanceList80, requestedSlots, name, domainString, bookinginfo=None):
...
def _getInformAlternativeEntities(ontology, acceptanceList, acceptanceList80, prohibitedList, domainString):
    '''
...
def _getInformMoreEntity(ontology, name, domainString): #ic340 is this ever used? apparently not
    '''
...
def add_venue_count(ontology, input, belief, domainString):
    '''Add venue count.
    ...
    count = ontology.get_length_entity_by_features(domainString, accepted_slots)
