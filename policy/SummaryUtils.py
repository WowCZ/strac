...
def globalSummary(ontology, belief, domainString):
    '''
    ...
    summaryArray['GLOBAL_COUNTACCEPTED'] = len(getTopBeliefs(ontology, belief, domainString=domainString))
...
def arraySlotSummary(ontology, belief, domainString):
    '''
    ...
    '''
    summary = {}
    slots = ontology.get_sorted_system_requestable_slots(domainString)
...
def getTopBeliefs(ontology, belief, threshold='auto', domainString=None):
    '''
    ...
    '''
    top_beliefs = {}
    for slot in ontology.get_system_requestable_slots(domainString):
        if threshold == 'auto':
            numvalues = ontology.get_len_informable_slot(domainString, slot)
...
def acceptanceListCanBeDiscriminated(ontology, accepted_values, domainString, num_accepted=None):
    '''
    ...
    return ontology.constraintsCanBeDiscriminated(domainString, constraints=ordered_accepted_values)
...
def getInformByConstraints(ontology, constraints, domainString, lastInformedVenue, SetObj):
    '''
    ...
    '''
    entities = ontology.entity_by_features(domainString, constraints)
    if len(entities) == 0:
        ...
        for ent in entities:
            if ent['name'] == lastInformedVenue:
                ret_ent = ent
                break
        return getInformEntity(constraints, ret_ent, SetObj)


def getInformEntity(accepted_values, ent, SetObj):
    '''
    ...
    maxNumFeats = 5
    if SetObj.config.has_option("summaryacts", "maxinformslots"):
        maxNumFeats = int(SetObj.config.get('summaryacts', 'maxinformslots'))

    if numFeats > maxNumFeats:
        SetObj.random.shuffle(acceptance_keys)
...
def getInformRequestedSlots(ontology, requested_slots, name, domainString, SetObj):
    '''
    ...
    :return: string representing the inform dialogue act
    '''
    result = ontology.entity_by_features(domainString, {'name': name})

    if len(result) > 0:
        ent = result[0]
        return _getInformRequestedSlotsForEntity(ontology, requested_slots, ent, domainString, SetObj)
    else:
        if not name:
            # Return a random venue
            result = []
            while len(result) == 0:
                rand_name = ontology.getRandomValueForSlot(domainString, 'name', nodontcare=True)
                result = ontology.entity_by_features(domainString, {'name': rand_name})
            ent = result[0]
            return _getInformRequestedSlotsForEntity(ontology, requested_slots, ent, domainString, SetObj)
...
def _getInformRequestedSlotsForEntity(ontology, requested_slots, ent, domainString, SetObj):
    '''
    ...
    if len(requested_slots) == 0:
        if 'type' in ent:
            ...
            slots = ontology.get_requestable_slots(domainString)
            if 'name' in slots:
                slots.remove('name')
            slot = slots[SetObj.random.randint(len(slots))]
            slotvaluepair.append('{}="{}"'.format(slot, ent[slot]))

    else:
        max_num_feats = 5
        if SetObj.config.has_option("summaryacts", "maxinformslots"):
            max_num_feats = int(SetObj.config.get('summaryacts', 'maxinformslots'))

        if len(requested_slots) > max_num_feats:
            SetObj.random.shuffle(requested_slots)
...
def getInformAlternativeEntities(ontology, accepted_values, prohibited_list, domainString, SetObj):
    ...
    result = ontology.entity_by_features(domainString, constraints)
    if len(result) == 0:
        return getInformNoneVenue(constraints)
    else:
        for ent in result:
            ...
            if name not in prohibited_list:
                return getInformEntity(accepted_values, ent, SetObj)

        return getInformNoMoreVenues(accepted_values, result, SetObj)


def getInformNoMoreVenues(accepted_values, entities, SetObj):
    '''
    ...
    if len(feats) > maxNumFeats:
        ...
        truncated_feats = {}
        SetObj.random.shuffle(feats_keys)
...
def actionSpecificInformSummary(ontology, belief, numAccepted, domainString):
    ...
    '''
    acceptanceList = getTopBeliefs(ontology, belief, domainString=domainString)
    count = _countEntitiesForAcceptanceListPart(ontology, acceptanceList, numAccepted, domainString)
    discriminatable = acceptanceListCanBeDiscriminated(ontology, acceptanceList, domainString, numAccepted)
    summary_array = [count == 0, count == 1, 2 <= count <= 4, count > 4, discriminatable]
    return summary_array

def _countEntitiesForAcceptanceListPart(ontology, accepted_values, num_accepted, domainString):
    '''
...
#     return len(ontology.entity_by_features(domainString, constraints=constraints))
    return ontology.get_length_entity_by_features(domainString, constraints=constraints)
