...
import DRL.hack_marainbow as dqn
import Policy
...
def flatten_belief(belief, domainUtil, logger, merge=False):
    ...
    #     for value in domainUtil.ontology['informable'][slot]:
    #         if slot == 'warranty' or value == 'warranty':
    #             print('slot: '+slot)
    #             print('value: '+value)
    # print(belief)
    # exit(0)
...
class RBDQNPolicy(Policy.Policy):
    ...
    def record(self, reward, domainInControl=None, weight=None, state=None, action=None):
        ...
        # print 'recording...'
        cur_action_q = self.dqn.predict(cur_cState, self.support)
        cur_target_q = self.dqn.predict_target(cur_cState, self.support)
    ...
    def nextAction(self, beliefstate):
        ...
        if self.exploration_type == 'e-greedy':
            ...
            if self.is_training and self.SetObj.random.rand() < self.epsilon:
                ...
                # print 'predicting...'
                action_Q = self.dqn.predict(np.reshape(beliefVec, (1, len(beliefVec))), self.support)  # + (1. / (1. + i + j))
                admissible = np.add(action_Q, np.array(execMask))
                self.logger.info('action Q...')
                #print admissible
        ...
        # print 'summaryAct:', summaryAct
        # print 'masterAct:', masterAct
        return masterAct, nextaIdex

    def _sample_and_updateQ(self):
        ...
        pns = self.dqn.get_distribution(s2_batch) 
        # self.dqn.reset_target_noise()
        target_pns = self.dqn.get_target_distribution(s2_batch)
        tmp = np.vstack([np.expand_dims(self.support, 0)]*pns.shape[1])
        support_exp = np.vstack([np.expand_dims(tmp, 0)]*pns.shape[0])
        action_q = np.sum(support_exp * pns, axis=2)  
        target_q = np.sum(support_exp * target_pns, axis=2)
         

        target_dis = []
        for k in xrange(min(self.minibatch_size, self.episodes[self.domainString].size())):
            if t_batch[k]:
                Tz = r_batch[k] + np.zeros_like(self.support)
            else:
                Tz = r_batch[k] + self.gamma * self.support

            np.clip(Tz, self.vmin, self.vmax, out=Tz)
            b = (Tz - float(self.vmin)) / self.delta_z  
            ...
            if self.q_update == 'single':
                ...
                action_Q = target_q[k] 
                admissible = np.add(action_Q, np.array(execMask))
                next_action_indx = np.argmax(admissible)
                Q_bootstrap_label = r_batch[k] + self.gamma * action_Q[next_action_indx]
            elif self.q_update == 'double':
                execMask = self.summaryaction.getExecutableMask(s2_ori_batch[k], a_batch[k])
                action_Q = action_q[k]
                admissible = np.add(action_Q, np.array(execMask))
            ...
            target_pns_a = target_pns[k][next_action_indx]
            ...
            for i in range(self.atoms):
                if int(l[i]) == 0:
                    m[int(l[i])] = m[int(l[i])] + target_pns_a[i]*(float(u[i])-b[i])
                elif int(u[i]) == self.atoms:
                    m[int(u[i])-1] = m[int(u[i])-1] + target_pns_a[i]*(b[i]-float(l[i]))
                else:
                    m[int(l[i])] = m[int(l[i])] + target_pns_a[i]*(float(u[i])-b[i])
                    m[int(u[i])] = m[int(u[i])] + target_pns_a[i]*(b[i]-float(l[i]))
        ...
        a_batch_one_hot = np.array([np.expand_dims(x, 2) for x in a_batch_one_hot])

        # Update the critic given the targets
        reshaped_targetdis = np.vstack([np.expand_dims(x, 0) for x in target_dis])

        return s_batch, a_batch_one_hot, reshaped_targetdis
    ...
    def _load_and_update(self, grads_list, batch_size):
        ...
        values = []
        for name, value in self.dqn.qnet.collect_params().items():
        ...
        self.dqn.trainer.step(batch_size=batch_size, ignore_stale_grad=True)

    def _savePolicy(self, FORCE_SAVE=False):
        """
        ...
        # lock.acquire()
        self.dqn.save_network(filename + '.dqn.ckpt')
        # lock.release()
        # CThread = str(threading.currentThread().getName())
        # print(CThread + ' save latest policy')

    def _savePolicyInc(self, FORCE_SAVE=False):
        """
        save model and replay buffer
        """
        # print('Save the latest parameters...')
        self._savePolicy(FORCE_SAVE=False)
    ...
    def saveTrainerAndReplay(self):
        ...
        # print('Saving deepq-network periodically in ' + self.domainString)
        # lock.acquire()
        self.dqn.save_trainer(self.out_policy_file + '.dqn.ckpt')
        # lock.release()
    ...
    def loadLastestPolicy(self):
        ...
        filename = '/'.join(tmp)
        # lock.acquire()
        self.dqn.load_test_network(filename + '.dqn.ckpt')
        # lock.release()
    ...
    def loadPolicy(self, filename):
        ...
        try:
            ...
            self.logger.info("Loading both model from %s and replay buffer..." % filename)
            f.close()
        except:
