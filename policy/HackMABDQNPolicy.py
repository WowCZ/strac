...
import DRL.hack_mabdqn as bbqn
import Policy
...
class BDQNPolicy(Policy.Policy):
    ...
    def __init__(self, in_policy_file, out_policy_file, domainString='CamRestaurants', is_training=False):
        ...
        self.sigma_prior = nd.exp(nd.array([sigma_prior,]))  # np.exp(np.array(sigma_prior, dtype=np.float32))
        ...
        self.madqn_n_hidden_layers = None
        if cfg.has_option('dqnpolicy', 'madqn_n_hidden_layers'):
            self.madqn_n_hidden_layers = cfg.getint('dqnpolicy', 'madqn_n_hidden_layers')
        ...
        self.nature_mode = False
        if cfg.has_option('dqnpolicy', 'nature_mode'):
            self.nature_mode = cfg.getboolean('dqnpolicy', 'nature_mode')

        self.bbb_map_mode = False
        if cfg.has_option('dqnpolicy', 'bbb_map_mode'):
            self.bbb_map_mode = cfg.getboolean('dqnpolicy', 'bbb_map_mode')
        ...
        with tf.device("/cpu:0"):
            ...
            self.bbqn = bbqn.DeepQNetwork(self.domainString, self.state_dim, self.action_dim,
                                          self.learning_rate, self.tau, action_bound, self.architecture,
                                          self.h1_size, self.h2_size, self.n_samples, self.minibatch_size,
                                          self.sigma_prior, self.n_batches, self.stddev_var_mu,
                                          self.stddev_var_logsigma, self.mean_log_sigma,
                                          self.importance_sampling, self.alpha_divergence,
                                          self.alpha, self.sigma_eps,
                                          n_hidden_layers=self.madqn_n_hidden_layers,
                                          local_hidden_units=self.madqn_local_hidden_units,
                                          global_hidden_units=self.madqn_global_hidden_units,
                                          map_mode=self.bbb_map_mode)
    ...
    def train(self):
	    ...
	    #self.bbqn.update_target_network()
            # predicted_q_value, _, currentLoss, logLikelihood, varFC2, meanFC2, td_error, KL_div = \
            self.bbqn.train(s_batch, a_batch_one_hot, reshaped_yi)
