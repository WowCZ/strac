...
class SimulationSystem(object):
    ...
    def __init__(self, error_rate, ontology, cfg, logger, SetObj):
        ...
        #-----------------------------------------
        self.agent_factory = Agent.AgentFactory(hub_id='simulate', ontology=ontology, cfg=cfg, logger=logger, SetObj=SetObj)
        ...
        # Simulated User.
        #-----------------------------------------
        self.simulator = SimulatedUsersManager.SimulatedUsersManager(error_rate, ontology, cfg, logger, SetObj)
        self.traceDialog = 2
        self.sim_level = 'act'
        self.logger = logger
    ...
    def run_dialogs(self, numDialogs, train=True):
        '''
        ...
        for i in range(numDialogs):
            self.logger.info('Dialogue %d' % (i+1))
            self.run(session_id='simulate_dialog'+str(i), sim_level=self.sim_level)
 
        self.agent_factory.power_down_factory(train) # Important! -uses FORCE_SAVE on policy- which will finalise learning and save policy.
    ...
    def run(self, session_id, agent_id='Smith', sim_level='dial_act'):
        ...
        for domain in self.simulator.simUserManagers:
            if self.simulator.simUserManagers[domain]:
                ...
                self.logger.dial('User will execute the following goal: {}'
        ...
        # print(threading.currentThread().getName()+' ************ '+ str(id(self.agent_factory)) + '\n')
        ...
        if prompt_str is not None:      # if we are generating text, versus remaining only at semantic level.
            ...
            self.logger.info('| Prompt > '+ prompt_str)
        # print(curThreadName+ ' | Prompt > '+ str(sys_act))
        # LOOP OVER TURNS:
        while not endingDialogue:
            ...
            if self.traceDialog>1:
                print '   User >', user_act
            self.logger.dial('| User > ' + user_act.to_string())
            # print(curThreadName+ ' | User > ' + user_act.to_string())
        ...
        for domain in self.simulator.simUserManagers:
            if self.simulator.simUserManagers[domain]:
                goal = self.simulator.simUserManagers[domain].um.goal
                self.logger.dial('User goal at the end of the dialogue: {}'
                            .format(str(goal.request_type) + str(goal.constraints) + str([req for req in goal.requests])))
